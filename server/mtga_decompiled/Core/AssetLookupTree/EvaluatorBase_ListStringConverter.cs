...
public class EvaluatorBase_ListStringConverter : JsonConverter
{
	private readonly StringCache _stringCache;

	public EvaluatorBase_ListStringConverter(StringCache stringCache)
	{
	...
	{
		EvaluatorBase_List<string> evaluatorBase_List = (existingValue as EvaluatorBase_List<string>) ?? ((EvaluatorBase_List<string>)objectType.GetConstructor(Type.EmptyTypes).Invoke(Array.Empty<object>()));
		while (reader.Read() && reader.TokenType != JsonToken.EndObject)
		{
			...
			case "ExpectedValues":
				...
				{
					evaluatorBase_List.ExpectedValues.Add(_stringCache.Get((string)reader.Value));
				}
				continue;
			case "Operation":
			...
			throw new InvalidOperationException("EvaluatorBase_ListStringConverter parsing error, unhandled property: " + text);
		}
	...
	{
		return objectType.BaseType == typeof(EvaluatorBase_List<string>);
	}
}
