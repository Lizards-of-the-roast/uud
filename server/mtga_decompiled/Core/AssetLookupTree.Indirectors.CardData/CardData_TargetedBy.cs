...
public class CardData_TargetedBy : IIndirector
{
	...
	public IEnumerable<IBlackboard> Execute(IBlackboard bb)
	{
		...
		foreach (MtgEntity item in bb.CardData.TargetedBy)
		{
			if (!(item is MtgCardInstance instance))
			{
				if (item is MtgPlayer player)
				{
					bb.SetCardDataRaw(_cacheCardData);
					bb.CardHolderType = _cacheHolderType;
					bb.Player = player;
