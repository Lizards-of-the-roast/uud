...
public class DeclareBlockersTranslation_NPE : IWorkflowTranslation<DeclareBlockersRequest>
{
	...
	public DeclareBlockersTranslation_NPE(NPEDirector npeDirector, IContext context, UIManager uiManager)
		: this(npeDirector, context.Get<IObjectPool>(), context.Get<IClientLocProvider>(), context.Get<IPromptEngine>(), context.Get<IGameStateProvider>(), context.Get<ICardHolderProvider>(), context.Get<ICardViewProvider>(), uiManager)
	{
	}

	private DeclareBlockersTranslation_NPE(NPEDirector npeDirector, IObjectPool objectPool, IClientLocProvider clientLocProvider, IPromptEngine promptEngine, IGameStateProvider gameStateProvider, ICardHolderProvider cardHolderProvider, ICardViewProvider cardViewProvider, UIManager uiManager)
	{
	...
	{
		return new DeclareBlockersWorkflow_NPE(req, _npeDirector, _objPool, _clientLocProvider, _promptEngine, _gameStateProvider, _cardViewProvider, GetBattlefield(), _uiManager);
	}
