...
[Serializable]
public class GemReward : AmountReward<RewardDisplay>
{
	protected override RewardType _rewardType => RewardType.Gem;
	...
	{
		AddIfPositive(cache.GemsLessCards);
	}
	...
	public override IEnumerable<Func<RewardDisplayContext, IEnumerator>> DisplayRewards(ContentControllerRewards ccr)
	{
		...
		if (toAddCount > 0)
		{
			yield return (RewardDisplayContext ctxt) => DisplayAmountRewardPile(ccr, ctxt.ChildIndex, WwiseEvents.sfx_ui_main_rewards_gem_flipout, toAddCount);
		}
	}
}
