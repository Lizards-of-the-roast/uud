...
Permanent_State From_Proto(const mtg::proto::PermanentState &proto) {
    ...
    Card card = From_Proto(proto.card());
    perm.card = card.instance_id;
    instances.Upsert(std::move(card));
...
Player_State From_Proto(const mtg::proto::PlayerState &proto) {
    ...
    for (const auto &c : proto.hand()) {
        Card card = From_Proto(c);
        player.hand.push_back(card.instance_id);
        instances.Upsert(std::move(card));
    }
    for (const auto &p : proto.battlefield()) {
        Permanent_State perm = From_Proto(p);
        player.battlefield.push_back(perm.permanent_id);
        instances.Upsert(std::move(perm));
    }
    for (const auto &c : proto.graveyard()) {
        Card card = From_Proto(c);
        player.graveyard.push_back(card.instance_id);
        instances.Upsert(std::move(card));
    }
    for (const auto &c : proto.exile()) {
        Card card = From_Proto(c);
        player.exile.push_back(card.instance_id);
        instances.Upsert(std::move(card));
    }
    player.has_priority = proto.has_priority();
    player.lands_played_this_turn = proto.lands_played_this_turn();
    player.clock_remaining_ms = proto.clock_remaining_ms();
    return player;
}
...
Action_Prompt From_Proto(const mtg::proto::ActionPrompt &proto) {
    ...
    switch (proto.prompt_type_case()) {
        ...
        case mtg::proto::ActionPrompt::kColorChoice: {
            Color_Choice_Prompt cc;
            cc.reason = proto.color_choice().reason();
            for (const auto &c : proto.color_choice().legal_colors())
                cc.legal_colors.push_back(c);
            prompt.prompt = cc;
            break;
        }
        case mtg::proto::ActionPrompt::kCreatureType: {
            Creature_Type_Prompt ct;
            ct.reason = proto.creature_type().reason();
            for (const auto &s : proto.creature_type().suggestions())
                ct.suggestions.push_back(s);
            prompt.prompt = ct;
            break;
        }
        ...
        case mtg::proto::ActionPrompt::kXCost: {
            X_Cost_Prompt xp;
            xp.card_name = proto.x_cost().card_name();
            xp.max_x = proto.x_cost().max_x();
            xp.x_count = proto.x_cost().x_count();
            prompt.prompt = xp;
            break;
        }
        case mtg::proto::ActionPrompt::kConcedeConfirm: {
            Concede_Confirm_Prompt cc;
            cc.message = proto.concede_confirm().message();
            prompt.prompt = cc;
            break;
        }
        default:
    ...
    switch (proto.event_case()) {
        ...
        case mtg::proto::GameEvent::kCardPlayed:
            ge.event = Card_Played_Event{proto.card_played().player_id(),
                                         ...
                                         proto.card_played().stack_entry_id()};
            break;
        case mtg::proto::GameEvent::kPermanentDestroyed:
            ge.event = Permanent_Destroyed_Event{proto.permanent_destroyed().permanent_id(),
                                                 ...
                                                 proto.permanent_destroyed().animation_hint()};
            break;
        case mtg::proto::GameEvent::kDamageDealt:
            ge.event =
                Damage_Dealt_Event{proto.damage_dealt().source_id(),
                                   proto.damage_dealt().target_id(),
                                   proto.damage_dealt().amount(),
                                   proto.damage_dealt().animation_hint()};
            break;
        ...
        case mtg::proto::GameEvent::kAbilityActivated:
            ge.event = Ability_Activated_Event{proto.ability_activated().permanent_id(),
                                               ...
                                               proto.ability_activated().stack_entry_id()};
            break;
        ...
        case mtg::proto::GameEvent::kZoneTransfer:
            ge.event = Zone_Transfer_Event{
                ...
                proto.zone_transfer().animation_hint()};
            break;
        ...
        case mtg::proto::GameEvent::kGameOver:
            ge.event = Game_Over_Event{proto.game_over().winner_id(),
                                       proto.game_over().reason(),
                                       proto.game_over().is_draw()};
            break;
        ...
        case mtg::proto::GameEvent::kActionPrompt:
            ...
            break;
        case mtg::proto::GameEvent::kDrawOffer:
            ge.event = Draw_Offer_Event{proto.draw_offer().from_player_id()};
            break;
        case mtg::proto::GameEvent::kDrawDeclined:
            ge.event = Draw_Declined_Event{proto.draw_declined().by_player_id()};
            break;
        case mtg::proto::GameEvent::kPermanentEntered: {
            const auto &peb = proto.permanent_entered();
            ge.event = Permanent_Entered_Battlefield_Event{
                peb.permanent_id(), peb.controller_id(), From_Proto(peb.card()), peb.tapped(),
                peb.is_token(), peb.animation_hint()};
        } break;
        case mtg::proto::GameEvent::kPlayerEliminated:
            ge.event = Player_Eliminated_Event{proto.player_eliminated().player_id(),
                                                proto.player_eliminated().reason()};
            break;
        case mtg::proto::GameEvent::kRopeWarning:
            ge.event = Rope_Warning_Event{proto.rope_warning().player_id(),
    ...
    std::visit(
        [&proto](const auto &a) {
            ...
            if constexpr (std::is_same_v<T, Pass_Priority_Action>) {
                ...
                auto *act = proto.mutable_play_card();
                act->set_card_instance_id(a.card_instance_id);
                act->set_x_value(a.x_value);
                act->set_kicked(a.kicked);
                act->set_flashback(a.flashback);
                for (auto id : a.convoke_ids)
                    act->add_convoke_ids(id);
                act->set_delve_count(a.delve_count);
            ...
            } else if constexpr (std::is_same_v<T, Draw_Offer_Action>) {
                proto.mutable_draw_offer();
            } else if constexpr (std::is_same_v<T, Draw_Response_Action>) {
                proto.mutable_draw_response()->set_accept(a.accept);
            } else if constexpr (std::is_same_v<T, Set_Auto_Pass_Action>) {
                proto.mutable_set_auto_pass()->set_mode(
                    static_cast<mtg::proto::AutoPassMode>(a.mode));
            }
        },
        action.action);
