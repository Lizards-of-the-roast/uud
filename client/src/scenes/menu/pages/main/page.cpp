...
bool Menu_Main_Page(Widget_Context &w, UI_Context &ui, Menu_Tab &tab) {
    ...
    if (!state.offline) {
        UI_Signal hist = w.Button("Match History");
        Offset_If_Hovered(hist, {val, 0});
        if (hist.flags & UI_SIG_LEFT_RELEASED)
            tab = (tab != Menu_Tab::Match_History) ? Menu_Tab::Match_History : Menu_Tab::None;
    }
