...
public:
    ...
    // library/graveyard
    virtual void scry(uint64_t player_id, int count) = 0;
    ...
    virtual void animate(uint64_t permanent_id, int power, int toughness,
                         bool until_eot = true) = 0;
    ...
    virtual void counter_spell(uint64_t stack_entry_id) = 0;
    [[nodiscard]] virtual auto choose_sacrifice(uint64_t player_id, const std::string& filter)
        -> uint64_t = 0;
