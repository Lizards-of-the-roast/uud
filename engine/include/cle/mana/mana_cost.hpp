...
struct ManaCost {
    ...
    std::vector<ManaColor> phyrexian_costs;
    ...
    [[nodiscard]] auto mana_value() const noexcept -> int {
        ...
        total += static_cast<int>(phyrexian_costs.size());
...
private:
    auto apply_token(std::string_view token) -> std::expected<void, ManaParseError> {
        ...
        if (token.size() == 3 && token[1] == '/' && (token[2] == 'P' || token[2] == 'p')) {
            auto color_result = char_to_color(token[0]);
            if (!color_result) {
                return std::unexpected(std::move(color_result).error());
            }
            phyrexian_costs.push_back(*color_result);
            return {};
        }
